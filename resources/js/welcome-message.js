(function () {
  ...
  const WELCOME_TEXT =
    ...
    'Gunakan tombol Spasi untuk pencarian suara, atau katakan "Hai Audio Statistik".';
  const MAAF =
    "Mohon maaf, untuk saat ini perangkat anda belum disupport oleh Audio Statistik ";
  ...
  function playWelcomeOnce() {
    ...
    (function () {
      ...
      const isiPad1 = /iPad/.test(ua); // iPad lama
      const isiPad2 = platform === "MacIntel" && isTouch; // iPadOS modern nyamar
      const isiPadOS = isiPad1 || isiPad2;
      const isMacOS = /Mac/.test(platform) && !isiPadOS;

      if (isiOS || isiPadOS || isMacOS) {
        // Hindari loop: jika sudah di /unsupported, jangan redirect lagi
        console.log("mac ipad or ios detected");
        appleDevice = true;
        if (!location.pathname.startsWith("/unsupported")) {
          window.location.replace("/unsupported");
        }
      }
      console.log("not mac ios ipad");
    })();
    ...
    if (!("speechSynthesis" in window)) {
      console.warn(
        "[Welcome] SpeechSynthesis not supported, skipping welcome."
      );
    ...
    if (window.AudioStatistik?.VoiceCoordinator?.requestWithPriority) {
      try {
        window.AudioStatistik.VoiceCoordinator.requestWithPriority(
          "welcome-message"
        );
      } catch (e) {
        /* ignore */
    ...
    utter.onend = () => {
      ...
      try {
        if (window.AudioStatistik?.VoiceCoordinator?.releaseRecognition) {
          window.AudioStatistik.VoiceCoordinator.releaseRecognition(
            "welcome-message"
          );
        }
    ...
    function scheduleStartCheck() {
      timeoutId = setTimeout(() => {
        if (!started && !window.speechSynthesis.speaking) {
          console.warn(
            "[Welcome] start not detected — likely blocked by autoplay policy. Waiting for first user gesture."
          );
          attemptInProgress = false;
          attachGestureListenerIfNeeded();
        }
  ...
  function attachGestureListenerIfNeeded() {
    ...
    document.addEventListener("pointerdown", handlerWrapper, {
      once: true,
      passive: true,
    });
  ...
  function removeGestureListener() {
    try {
      if (handlerWrapper) {
        document.removeEventListener("pointerdown", handlerWrapper, {
          once: true,
        });
        handlerWrapper = null;
      }
  ...
  if (document.readyState === "loading") {
    ...
    initWelcome(); // DOM sudah siap, langsung jalan
  }
})();
