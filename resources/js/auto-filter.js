...
export function initAutoFilter() {
  ...
  function updateGridContent(doc) {
    ...
    if (currentGrid && newGrid) {
      ...
      const noResultsSection = doc.querySelector(
        ".text-center.py-16, .text-center.py-12"
      );
  ...
  function initializeDocumentCards() {
    ...
    document.querySelectorAll(".play-document-btn").forEach((button) => {
      button.addEventListener("click", function (e) {
        ...
        try {
          ...
          if (window.playDocumentAudio) {
            ...
            console.warn("⚠️ playDocumentAudio function not found on window.");
          }
        } catch (error) {
  ...
  function initializePaginationLinks() {
    document
      .querySelectorAll(".flex.justify-center a[href]")
      .forEach((link) => {
  ...
  function showLoadingIndicator() {
    ...
    indicator.className =
      "fixed top-20 right-4 bg-blue-600 text-white px-4 py-2 rounded-lg shadow-lg z-50 flex items-center space-x-2";
    indicator.innerHTML = `
  ...
  if (yearSelect) {
    yearSelect.addEventListener("change", function () {
      ...
      announceToScreenReader(
        `Filter tahun diubah ke: ${selectedValue || "Semua tahun"}`
      );
    });
  }
  ...
  function addClearFiltersButton() {
    ...
    if (hasActiveFilters && filterForm) {
      ...
      clearBtn.className =
        "ml-2 px-4 py-2 bg-gray-500 hover:bg-gray-600 text-white text-sm font-medium rounded-lg transition-colors hover-sound";
      clearBtn.innerHTML =
        '<i class="fas fa-times mr-2" aria-hidden="true"></i><span class="text-sound">Reset Filter</span>';
    ...
    input.addEventListener("focus", function () {
      if (!suggestionsContainer) {
        ...
        suggestionsContainer.className =
          "absolute top-full left-0 right-0 bg-white border border-gray-300 rounded-b-lg shadow-lg z-10 hidden";
  ...
  document.addEventListener("keydown", function (e) {
    ...
    // console.log({ sdsd: e.target.tagName });
  ...
  function showFilterStatus() {
    ...
    if (params.get("search"))
      activeFilters.push(`Pencarian: "${params.get("search")}"`);
    if (params.get("query"))
      activeFilters.push(`Kata kunci: "${params.get("query")}"`);
    if (params.get("type")) {
      const typeText =
        params.get("type") === "publication" ? "Publikasi" : "BRS";
    ...
    if (params.get("indicator")) {
      const indicatorElement = document.querySelector(
        `option[value="${params.get("indicator")}"]`
      );
      if (indicatorElement) {
