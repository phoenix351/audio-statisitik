...
<div class="max-w-7xl mx-auto px-4 sm:px-6 lg:px-8 py-8">
    ...
    <div class="bg-white rounded-lg shadow-sm p-6 mb-8">
        <div class="flex items-center justify-between mb-6">
            <div>
                ...
                <p class="text-sm text-gray-600 mt-1">Total: {{ $totalApiKeys }} keys | Showing {{ $currentPageStart }}-{{ $currentPageEnd }}</p>
            </div>
                ...
                <div class="border border-gray-200 rounded-lg p-4 hover:shadow-md transition-shadow">
                    <div class="flex items-center justify-between mb-3">
                        <div class="flex items-center space-x-3">
                            <div class="w-4 h-4 rounded-full {{ $api['is_active'] ? 'bg-green-500' : 'bg-red-500' }}"></div>
                            <div>
                    ...
                    <div class="flex items-center justify-between text-sm">
                        ...
                        <button onclick="testApiKey({{ $api['index'] }})" 
    ...
    <div class="grid grid-cols-1 lg:grid-cols-2 gap-8 mb-8">
        ...
        <div class="bg-white rounded-lg shadow-sm p-6">
            <div class="flex items-center justify-between mb-4">
                ...
                <button onclick="refreshQueueStats()" class="text-blue-600 hover:text-blue-800 text-sm">
            ...
            <div class="space-y-4">
                <div class="flex items-center justify-between p-3 bg-gray-50 rounded-lg">
                    ...
                    <span class="font-bold text-lg {{ $queueStats['pending_jobs'] > 10 ? 'text-orange-600' : 'text-gray-900' }}">
                ...
                <div class="flex items-center justify-between p-3 bg-gray-50 rounded-lg">
                    ...
                    <span class="font-bold text-lg {{ $queueStats['failed_jobs'] > 5 ? 'text-red-600' : 'text-gray-900' }}">
                ...
                <div class="flex items-center justify-between p-3 bg-gray-50 rounded-lg">
                    ...
                    <span class="font-bold text-lg {{ $queueStats['failed_last_hour'] > 2 ? 'text-red-600' : 'text-gray-900' }}">
        ...
        <div class="bg-white rounded-lg shadow-sm p-6">
            ...
            <div class="mt-4 p-3 bg-orange-50 border border-orange-200 rounded-lg">
                <div class="flex items-center justify-between">
                    ...
                    <button onclick="resetStuckDocuments()" class="text-orange-600 hover:text-orange-800 text-sm underline">
                        Reset Now
                    </button>
                </div>
            </div>
    ...
    <div class="bg-white rounded-lg shadow-sm p-6">
        ...
        <div class="grid grid-cols-1 md:grid-cols-3 gap-4">
            <button onclick="runCommand('api:check-status')" 
                    class="p-4 border border-gray-300 rounded-lg hover:bg-gray-50 hover:border-blue-300 text-left transition-colors">
            ...
            <button onclick="runCommand('documents:process-stuck')" 
                    class="p-4 border border-gray-300 rounded-lg hover:bg-gray-50 hover:border-orange-300 text-left transition-colors">
                <div class="flex items-center mb-2">
            ...
            <button onclick="runCommand('queue:restart')" 
                    class="p-4 border border-gray-300 rounded-lg hover:bg-gray-50 hover:border-green-300 text-left transition-colors">
                <div class="flex items-center mb-2">
...
<script>
    ...
    async function testApiKey(keyIndex) {
        ...
        try {
            const response = await fetch('/admin/api-monitor/test-key', {
                method: 'POST',
                headers: {
            ...
            if (result.status === 'active') {
                showToast(`API Key #${keyIndex + 1}: Working properly`, 'success');
            } else {
                showToast(`API Key #${keyIndex + 1}: ${result.error || 'Failed to connect'}`, 'error');
            }
            ...
            showToast(`Error testing API Key #${keyIndex + 1}: ${error.message}`, 'error');
        } finally {
    ...
    async function testAllKeys() {
        ...
        // Show loading state
        button.innerHTML = '<i class="fas fa-spinner fa-spin mr-2"></i>Testing All...';
        button.disabled = true;
        
        try {
            const response = await fetch('/admin/api-monitor/test-all-keys', {
                method: 'POST',
                headers: {
                    'Content-Type': 'application/json',
                    'X-CSRF-TOKEN': document.querySelector('meta[name="csrf-token"]').content
            ...
            const result = await response.json();
            showToast(`Tested ${result.total} keys. ${result.active} active, ${result.failed} failed.`, 'info');
            
            // Refresh the page to show updated status
            setTimeout(() => window.location.reload(), 2000);
            
        } catch (error) {
            showToast(`Error testing all keys: ${error.message}`, 'error');
        } finally {
            // Restore button state
            button.innerHTML = originalContent;
            button.disabled = false;
    ...
    async function resetStuckDocuments() {
        if (!confirm('Are you sure you want to reset all stuck documents? This will requeue them for processing.')) {
            return;
        }

        try {
            const response = await fetch('/admin/api-monitor/reset-stuck', {
                method: 'POST',
                headers: {
                    'Content-Type': 'application/json',
                    'X-CSRF-TOKEN': document.querySelector('meta[name="csrf-token"]').content
            ...
            showToast(`Error resetting documents: ${error.message}`, 'error');
        }
    }

    function runCommand(command) {
        document.getElementById('command-modal').classList.remove('hidden');
        document.getElementById('command-output').innerHTML = '<div class="animate-pulse">Running command: ' + command + '...</div>';
        
        // This would typically make an AJAX call to run the command
    ...
    function showFailedJobs() {
        // This could open a modal or redirect to a detailed failed jobs page
        window.open('/admin/failed-jobs', '_blank');
    }

    async function refreshQueueStats() {
        try {
            // Implement AJAX call to refresh just queue stats
            showToast('Queue stats refreshed', 'success');
        } catch (error) {
            showToast('Failed to refresh queue stats', 'error');
        }
    }

    // Toast notification utility (assuming it's available from the main layout)
    function showToast(message, type = 'info') {
        // This function should be defined in the main layout
