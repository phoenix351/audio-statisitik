...
<div class="max-w-7xl mx-auto px-4 sm:px-6 lg:px-8 py-8">
    ...
    <div class="flex items-center justify-between mb-8">
        <div>
            ...
            <p class="text-gray-600 mt-2 text-hover">Upload dan kelola dokumen publikasi serta BRS dengan konversi audio otomatis</p>
        </div>
    ...
    <div class="bg-white rounded-lg shadow-sm p-6 mb-6">
        <form method="GET" action="{{ route('admin.documents.index') }}" class="flex flex-wrap gap-4">
            <div>
                ...
                <select name="type" id="type" class="px-3 py-2 border border-gray-300 rounded-md text-sm focus:ring-2 focus:ring-blue-500 focus:border-blue-500 hover-sound">
            ...
            <div>
                ...
                <select name="status" id="status" class="px-3 py-2 border border-gray-300 rounded-md text-sm focus:ring-2 focus:ring-blue-500 focus:border-blue-500 hover-sound">
                    <option value="">Semua</option>
            ...
            <div class="flex items-end">
                <button type="submit" class="px-4 py-2 bg-gray-600 hover:bg-gray-700 text-white text-sm font-medium rounded-md transition-colors hover-sound">
                        ...
                        <tr class="hover:bg-gray-50">
                            <td class="px-6 py-4">
                                <div class="flex items-center">
                                    ...
                                    <div>
                                        <div class="text-sm font-medium text-gray-900 text-hover">{{ Str::limit($document->title, 50) }}</div>
                                        <div class="text-sm text-gray-500 text-hover">{{ $document->creator->name ?? 'Unknown' }}</div>
                                    </div>
                                </div>
                            </td>
                            <td class="px-6 py-4">
                            ...
                            <td class="px-6 py-4">
                                <span class="inline-flex items-center px-2 py-1 rounded-full text-xs font-medium
                                    ...
                                    @if($document->status === 'completed') <i class="fas fa-check mr-1" aria-hidden="true"></i>Selesai
                                    @elseif($document->status === 'processing') <i class="fas fa-spinner fa-spin mr-1" aria-hidden="true"></i>Diproses
                                    @elseif($document->status === 'failed') <i class="fas fa-times mr-1" aria-hidden="true"></i>Gagal
                                    @else <i class="fas fa-clock mr-1" aria-hidden="true"></i>Menunggu @endif
                                </span>
                            </td>
                            <td class="px-6 py-4 text-sm text-gray-900">
                                @if($document->hasAudio())
                                    <div class="flex items-center text-green-600">
                                        ...
                                        <span class="text-hover">{{ $document->getAudioDurationFormatted() }}</span>
                                    </div>
                                @else
                            ...
                            <td class="px-6 py-4 text-right text-sm font-medium">
                                <div class="flex items-center justify-end space-x-2">
                                    ...
                                    <form action="{{ route('admin.documents.reprocess', $document) }}" method="POST" class="inline">
                                        @csrf
                                        <button type="submit" 
            ...
            newButton.addEventListener('click', function(e) {
                ...
                try {
                    ...
                    if (documentData.indicator_id && !documentData.indicator) {
                        ...
                        const indicatorCell = this.closest('tr')?.querySelector('td:nth-child(3)');
                        const indicatorName = indicatorCell ? indicatorCell.textContent.trim() : 'Unknown';
                    ...
                    alert('Terjadi kesalahan saat memutar audio. Silakan refresh halaman dan coba lagi.');
                }
            });
    ...
    function tryFallbackAudioPlay(docData) {
        ...
        try {
            ...
            audio.addEventListener('error', (e) => {
                ...
                showSimpleNotification('Gagal memuat audio. Periksa koneksi internet Anda.', 'error');
            });
    ...
    function initializeTextHoverSounds() {
        ...
        textElements.forEach(element => {
            element.addEventListener('mouseenter', function() {
                if (window.playTextHoverSound && typeof window.playTextHoverSound === 'function') {
    ...
    window.addEventListener('error', function(e) {
        if (e.message && e.message.includes('getElementById')) {
            ...
            if (e.message.includes('playDocumentAudio') && !window.audioErrorNotified) {
                ...
                setTimeout(() => {
                    showSimpleNotification('Terjadi masalah dengan sistem audio. Silakan refresh halaman.', 'error');
                }, 100);
            }
        }
    });
