...
<div class="max-w-7xl mx-auto px-4 sm:px-6 lg:px-8 py-8">
    ...
    <div class="mb-8">
        ...
        <p class="text-gray-600 mt-2">Selamat datang, {{ auth()->user()->name }}! Kelola dokumen dan pantau statistik penggunaan Audio Statistik</p>
    </div>
        ...
        <div class="bg-white rounded-lg shadow-sm p-6 hover-sound">
            <div class="flex items-center">
                ...
                <div class="ml-4">
                    ...
                    <p class="text-2xl font-bold text-gray-900">{{ number_format($statistics['total_visitors'] ?? 0) }}</p>
                </div>
            </div>
        </div>
        ...
        <div class="bg-white rounded-lg shadow-sm p-6 hover-sound">
            <div class="flex items-center">
                ...
                <div class="ml-4">
                    ...
                    <p class="text-2xl font-bold text-gray-900">{{ number_format($statistics['total_publications'] ?? 0) }}</p>
                </div>
            </div>
        </div>
        ...
        <div class="bg-white rounded-lg shadow-sm p-6 hover-sound">
            <div class="flex items-center">
                ...
                <div class="ml-4">
                    ...
                    <p class="text-2xl font-bold text-gray-900">{{ number_format($statistics['total_audio_files'] ?? 0) }}</p>
                </div>
            </div>
        </div>
        ...
        <div class="bg-white rounded-lg shadow-sm p-6 hover-sound">
            <div class="flex items-center">
                ...
                <div class="ml-4">
                    ...
                    <p class="text-2xl font-bold text-gray-900">{{ number_format($statistics['pending_conversions'] ?? 0) }}</p>
                </div>
            </div>
        </div>
        ...
        <div class="bg-white rounded-lg shadow-sm p-6 hover-sound">
            <div class="flex items-center">
                ...
                <div class="ml-4">
                    ...
                    <p class="text-2xl font-bold text-gray-900">{{ number_format($statistics['failed_conversions'] ?? 0) }}</p>
                </div>
            </div>
        </div>
    </div>
    ...
    <div class="grid grid-cols-1 gap-8">     
        ...
        <div class="bg-white rounded-lg shadow-sm p-6">
            <div class="flex items-center justify-between mb-4">
                ...
                <a href="{{ route('admin.documents.index') }}" class="text-sm text-blue-600 hover:text-blue-700 hover-sound">Lihat semua</a>
            </div>
            <div class="space-y-4">
                ...
                <div class="flex items-center space-x-3 p-3 border border-gray-200 rounded-lg hover-sound">
                    <div class="flex-shrink-0">
                        <div class="w-8 h-8 bg-{{ $doc->type === 'publication' ? 'blue' : 'green' }}-100 rounded-lg flex items-center justify-center">
                            <i class="fas fa-{{ $doc->type === 'publication' ? 'book' : 'newspaper' }} text-{{ $doc->type === 'publication' ? 'blue' : 'green' }}-600 text-xs"></i>
                        </div>
                    </div>
                    <div class="flex-1 min-w-0">
                        <p class="font-medium text-gray-900 text-sm truncate">{{ Str::limit($doc->title, 100) }}</p>
                    ...
                    <div class="flex-shrink-0">
                        <span class="inline-flex items-center px-2 py-1 rounded-full text-xs font-medium
                            ...
                            @if($doc->status === 'completed') <i class="fas fa-check mr-1"></i>Selesai
                            @elseif($doc->status === 'processing') <i class="fas fa-spinner fa-spin mr-1"></i>Proses
                            @elseif($doc->status === 'failed') <i class="fas fa-times mr-1"></i>Gagal
                            @else <i class="fas fa-clock mr-1"></i>Menunggu @endif
                        </span>
                    </div>
                </div>
                @empty
                <div class="text-center py-8 text-gray-500">
                    ...
                    <a href="{{ route('admin.documents.create') }}" class="text-blue-600 hover:text-blue-700 font-medium hover-sound">
        ...
        <div class="bg-white rounded-lg shadow-sm p-6">
            ...
            <div class="space-y-3">
                ...
                <div class="flex items-center space-x-3 text-sm">
                    ...
                    <span class="text-gray-600">{{ $visitor->count ?? 0 }} pengunjung pada {{ $visitor->date ?? 'today' }}</span>
                </div>
                @empty
