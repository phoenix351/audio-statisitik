...
class DocumentController extends Controller
{
    ...
    public function streamAudio(Document $document, string $format)
    {
        // Ensure the document is playable
        if (!$document->is_active || $document->status !== 'completed') {
        ...
        if (!$disk->exists($relativePath)) {
            Log::warning('Audio file not found on disk', [
                'document_id' => $document->id,
                'format'      => $format,
                'path'        => $relativePath,
            ]);
        ...
        // If this is a local/visible disk, get the absolute filesystem path
        // and use BinaryFileResponse which supports Range requests.
        if (method_exists($disk, 'path')) {
            $absolutePath = $disk->path($relativePath);

            // Log basic info
            $size = @filesize($absolutePath);
            Log::info('Audio streaming (file)', [
                'document_id' => $document->id,
                'format'      => $format,
            ...
            // If NO Range header is present, count this as a full play.
            if (!request()->hasHeader('Range')) {
                $document->increment('play_count');
            }

            // Stream inline so the browser/audio tag can play it
            $response = response()->file($absolutePath, [
                'Content-Type'  => $mimeType,
                'Cache-Control' => 'public, max-age=3600',
                // Accept-Ranges is handled by BinaryFileResponse,
                // but adding it doesn't hurt:
                'Accept-Ranges' => 'bytes',
            ]);

            // Force inline (not "attachment") to ensure in-browser playback
        ...
        /**
         * Fallback for cloud disks (e.g., S3) where ->path() is unavailable:
         * Option A: Redirect to a (signed) URL. Range handling is then offloaded to the CDN/storage.
         * Option B: Manually proxy the stream with Range support (more complex with S3; you’d use the SDK’s Range option).
         *
         * Below is Option A (simple + reliable if your bucket/endpoint supports Range).
         */

        // Prefer a temporary signed URL if the files are private:
        if (method_exists($disk, 'temporaryUrl')) {
            $tmpUrl = $disk->temporaryUrl($relativePath, now()->addHour());
            // Full streams vs partial is tricky to count reliably when redirecting.
            // If you want an approximate count, increment here only on non-Range requests:
            if (!request()->hasHeader('Range')) {
                $document->increment('play_count');
            }
        ...
        // Otherwise, if files are publicly readable, use the public URL:
